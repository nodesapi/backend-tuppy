...
export class OrdersService {
  ...
  async checkout(dto: CreateOrderDto) {
    ...
    if (shippingCost && courier && destinationCityId) {
      if (!tenant.originCityId) {
        throw new BadRequestException(
          'Toko ini belum mengatur lokasi pengiriman.',
        );
      }
      ...
      for (const item of items) {
        if (!item.isDigital && item.productId) {
          // Wait, do we have productId in CreateOrderItemDto?
      ...
      if (productIds.length > 0) {
        ...
        totalWeight =
          1000 *
          items.reduce((sum, i) => (!i.isDigital ? sum + i.quantity : sum), 0);
      }

      if (
        totalWeight > 0 &&
        !courier.toLowerCase().includes('kirim langsung')
      ) {
        try {
          const costData = await this.shippingService.getCost(
            tenant.originCityId,
            destinationCityId,
            totalWeight,
            courier,
          ...
          if (costData && costData.length > 0 && costData[0].costs.length > 0) {
            ...
            const validCosts = costData[0].costs.map(
              (c: any) => c.cost[0].value,
            );
            if (!validCosts.includes(shippingCost)) {
              this.logger.warn(
                `Biaya pengiriman mismatch. Client: ${shippingCost}, Valid: ${validCosts.join(',')}`,
              );
        ...
        if (item.productId) {
          ...
          if (product && product.stock !== null) {
            if (product.stock < item.quantity) {
              throw new BadRequestException(
                `Stok produk "${product.title}" tidak mencukupi (sisa ${product.stock})`,
              );
            }
    ...
    try {
      ...
      this.logger.error(
        `Gagal mengaitkan order ke chat: ${(error as any)?.message || error}`,
      );
    }
  ...
  async getOrderDetail(orderId: string, userId: string) {
    ...
    if (
      ...
      const digitalItems = order.items.filter(
        (item: any) => item.isDigital && item.digitalFileId,
      );
      const secret =
        process.env.JWT_SECRET || 'tupply-secure-download-secret-12345';
  ...
  async updateOrderStatus(
    ...
    const updated = await this.prisma.order.update({
      ...
      data: {
        ...
        ...(dto.sellerPackingVideoUrl !== undefined && {
          sellerPackingVideoUrl: dto.sellerPackingVideoUrl,
        }),
        ...(dto.buyerUnboxingVideoUrl !== undefined && {
          buyerUnboxingVideoUrl: dto.buyerUnboxingVideoUrl,
        }),
      },
      include: { items: true },
    });
    ...
    if (
      (dto.status === 'PROCESSING' || dto.status === 'DELIVERED') &&
      dto.awbNumber &&
  ...
  async getOrderInvoice(orderNumber: string) {
    ...
    if (
      !order.paymentLink ||
      (order.paymentGateway !== 'PAYHOOK_GLOBAL' &&
        order.paymentGateway !== 'PAYHOOK_DEDICATED')
    ) {
      throw new BadRequestException(
        'Order ini tidak memiliki invoice otomatis.',
      );
    }
    ...
    if (order.paymentGateway === 'PAYHOOK_DEDICATED') {
      payhookInvoice = await this.payhookService.getInvoice(
        invoiceNumber,
        order.tenant.payhookApiKey || undefined,
      );
    } else {
    ...
    const finalInvoice = {
      ...(payhookInvoice || {}),
      payment_instruction:
        storedData.payment_instruction ||
        (payhookInvoice && payhookInvoice.payment_instruction) ||
        null,
      pay_amount:
        storedData.pay_amount ||
        (payhookInvoice && payhookInvoice.pay_amount) ||
        order.grandTotal,
    };
  ...
  async uploadPaymentProof(orderNumber: string, proofUrl: string) {
    ...
    if (
      order.tenant.notifMethod === 'WHATSAPP' ||
      order.tenant.notifMethod === 'BOTH'
    ) {
  ...
  private generateWaText(order: any, tenantPhone?: string | null) {
    const itemList = order.items
      .map(
        (i: any) =>
          `• ${i.productName} x${i.quantity} = Rp ${i.subtotal.toLocaleString('id-ID')}`,
      )
      .join('\n');
  ...
  async trackOrder(orderNumber: string, phone: string) {
    if (!orderNumber || !phone)
      throw new BadRequestException('Nomor pesanan dan WhatsApp wajib diisi');
  ...
  async confirmOrderGuest(
    ...
    if (order.status !== 'SHIPPED' && order.status !== 'DELIVERED') {
      throw new BadRequestException(
        'Pesanan belum dikirim, tidak dapat dikonfirmasi',
      );
    }
    ...
    if (isComplain) {
      if (!videoUrl)
        throw new BadRequestException(
          'Link video unboxing wajib diisi untuk komplain',
        );
      const updated = await this.prisma.order.update({
        where: { id: order.id },
        data: {
      ...
      if (order.tenant.waPhoneNumber) {
        this.whatsappService.sendMessage(
          order.tenantId,
          order.tenant.waPhoneNumber,
          `*[TUPPLY NOTIF]*\nPembeli mengajukan komplain untuk pesanan *${orderNumber}*. Silakan cek dashboard Anda.`,
          order.id,
        );
      }
  ...
  async downloadFile(token: string) {
    ...
    try {
      ...
      return {
        url: `${cdnUrl}${fileUrl.startsWith('/') ? '' : '/'}${fileUrl}`,
      };
    } catch (e: any) {
      this.logger.error(`Download failed: ${e.message}`);
      throw new BadRequestException(
        'Link download sudah kedaluwarsa atau tidak valid. Silakan muat ulang halaman (Refresh).',
      );
    }
  }
