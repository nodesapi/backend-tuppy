...
export class PayhookService {
  ...
  private async getConfig() {
    ...
    const dbConfig = config.reduce((acc, curr) => {
      acc[curr.key] = curr.value;
      return acc;
    }, {} as Record<string, string>);

    return {
      baseUrl: dbConfig['PAYHOOK_URL'] || process.env.PAYHOOK_URL || 'http://localhost:8000',
  ...
  async createInvoice(payload: {
    ...
    try {
      const response = await axios.post(`${config.baseUrl}/api/v1/invoices`, payload, {
        headers: {
  ...
  async provisionPayhookAccount(payload: {
    ...
    try {
      const config = await this.getConfig();
      const response = await axios.post(`${config.baseUrl}/api/internal/tupply/merchants`, payload, {
        headers: {
  ...
  async uploadQris(tenantId: string | number, file: Express.Multer.File) {
    try {
      ...
      const config = await this.getConfig();
      const response = await axios.post(`${config.baseUrl}/api/internal/tupply/merchants/${tenantId}/qris`, formData, {
        headers: {
          'X-Tupply-Secret': this.internalSecret,
          'Accept': 'application/json',
